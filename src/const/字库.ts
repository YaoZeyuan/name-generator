...
export const 常见字字库: { [key: string]: Type_Char_Item } = {}

// 为常见字字库赋值
for (let char of 常见字_list) {
  if (总字库[char]) {
    常见字字库[char] = 总字库[char]
  }
}
...
for (let char of Object.keys(总字库)) {
  let item = 总字库[char]
  if (Const_Pinyin_To_Char[item.pinyin]) {
    Const_Pinyin_To_Char[item.pinyin].char_list.push(char)
    continue
  } else {
    Const_Pinyin_To_Char[item.pinyin] = {
      pinyin: item.pinyin,
      tone: item.tone,
      char: item.char,
      char_list: [item.char],
...
Pinyin_List.sort((a, b) => {
  if (a.pinyin_without_tone === b.pinyin_without_tone) {
    return a.tone - b.tone
  } else {
    return a.pinyin_without_tone.localeCompare(b.pinyin_without_tone)
  }
})